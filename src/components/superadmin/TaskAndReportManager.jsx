    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-6 rounded-3xl border border-slate-800">
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex rounded-xl glass-card p-1 border border-slate-800 text-xs font-semibold">
            <button
      ...
      {activeTab === "reports" && (
        <div className="space-y-4">
          <h3 className="font-bold text-slate-100 text-base">Submitted End-of-Day Labour Reports</h3>
          ...
          <div className="space-y-4">
            {dailyReports.map((rep) => {
              ...
              return (
                <div key={rep.id} className="glass-card p-5 rounded-2xl border border-slate-800 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <div className="font-bold text-slate-100 text-sm">{emp?.name}</div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400 font-mono">
                      <span>Submitted: <strong>{rep.date}</strong></span>
                      <span className="px-2.5 py-0.5 rounded bg-sky-500/10 text-sky-400 font-bold uppercase text-[10px]">
                        {rep.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    <div className="space-y-2">
                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
                        <span className="text-[10px] font-bold text-emerald-400 uppercase block mb-1">Work Completed Today:</span>
                        <p className="text-slate-200 leading-relaxed">{rep.workCompleted}</p>
                      </div>

                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
                        <span className="text-[10px] font-bold text-amber-400 uppercase block mb-1">Pending Work:</span>
                        <p className="text-slate-300 leading-relaxed">{rep.workPending || "None"}</p>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
                        <span className="text-[10px] font-bold text-purple-400 uppercase block mb-1">Material Required for Tomorrow:</span>
                        <p className="text-purple-300 font-semibold">{rep.materialRequired || "None specified"}</p>
                      </div>

                      <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
                        <span className="text-[10px] font-bold text-rose-400 uppercase block mb-1">Problems / Site Issues Faced:</span>
                        <p className="text-slate-300">{rep.problemsFaced || "No site issues encountered"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {rep.photos && rep.photos.length > 0 && (
                    <div className="pt-2">
                      <span className="text-[10px] text-slate-400 block mb-2 font-bold uppercase">Submitted Site Work Photographs:</span>
                      <div className="flex flex-wrap gap-3">
                        {rep.photos.map((url, idx) => (
                          <img
                            ...
                            className="h-28 rounded-xl object-cover border border-slate-700 shadow"
                          />
                        ))}
                      </div>
                    </div>
      ...
      {activeTab === "tasks" && (
        <div className="space-y-4">
          <h3 className="font-bold text-slate-100 text-base">Active Site Task Board</h3>
              ...
              return (
                <div key={t.id} className="glass-card p-5 rounded-2xl border border-slate-800 space-y-3 flex flex-col justify-between">
                  <div>
                    ...
                    <h4 className="font-extrabold text-slate-100 text-sm mt-2">{t.title}</h4>
                    <p className="text-xs text-slate-400 mt-1 leading-relaxed">{t.description}</p>
                  </div>

                  <div className="pt-3 border-t border-slate-800 text-xs text-slate-400 flex items-center justify-between">
                    <span>Assigned To: <strong className="text-slate-200">{emp?.name || grp?.name || "Labour"}</strong></span>
                    <span className="font-mono text-amber-400">Due: {t.deadline}</span>
                  </div>
                </div>
              );
            })}
