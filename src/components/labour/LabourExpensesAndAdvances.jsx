  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 app-panel p-6 rounded-3xl">
        <div>
          <div className="flex items-center gap-2">
            <Receipt className="w-6 h-6 text-blue-600 dark:text-emerald-400" />
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-blue-600/20 transition-all self-start md:self-auto cursor-pointer"
        >
      ...
      <div className="app-card p-5 rounded-2xl flex items-center justify-between">
        <div>
          <span className="text-xs text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider">Total Personal Advances Received</span>
          <div className="text-2xl font-extrabold text-amber-600 dark:text-amber-400 font-mono mt-1">₹{totalAdvances}</div>
          <span className="text-[11px] text-slate-500 dark:text-slate-400">Deducted from monthly salary ledger</span>
        </div>
      </div>

      {/* Expense Vouchers List */}
      <div className="space-y-3">
        <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Expense & Advance History</h3>

        {myExpenses.length === 0 ? (
          <div className="app-card p-8 rounded-2xl text-center text-slate-500 dark:text-slate-400 text-xs">
            No expense or advance records found.
          </div>
        ) : (
          myExpenses.map((exp) => (
            <div key={exp.id} className="app-card p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm">{exp.purpose}</h4>
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Date: {exp.date}</div>
              </div>

              <div className="text-right">
                <div className="text-base font-mono font-extrabold text-amber-600 dark:text-amber-400">₹{exp.amount}</div>
                <span
                  className={`text-[10px] font-bold uppercase px-2.5 py-0.5 rounded ${
                    exp.status === "approved"
                      ? "bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-400 font-bold"
        ...
        <div className="modal-overlay">
          <div className="app-panel modal-dialog max-w-md shadow-2xl animate-in fade-in">
            <div className="modal-header p-5 border-b border-slate-200 dark:border-slate-800">
              <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Submit Expense Voucher</h3>
              <button
            ...
            <form onSubmit={handleSubmitVoucher} className="flex flex-col flex-1 min-h-0 overflow-hidden">
              <div className="modal-body p-5 space-y-3 text-xs">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Expense Purpose / Title *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Expense Amount (₹) *</label>
                  <input
                    type="number"
                    required
                    min="1"
                    ...
                    className="w-full app-input p-3 rounded-xl font-mono text-sm font-bold"
                  />
                </div>

                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Description / Details *</label>
                  <textarea
                    required
                    rows="3"
